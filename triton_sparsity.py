...
class TritonSparseMLP(nn.Module):
    ...
    def __init__(self, original_module: nn.Module, sparsity_ratio: float = 0.1, 
        ...
        Args:
            original_module: The original MLP module to replace (e.g., FFN in transformer)
            sparsity_ratio: Fraction of neurons to keep (e.g., 0.1 = keep 10%)
            k: Number of top neurons to keep (overrides sparsity_ratio if provided)
        ...
        self.gate_proj = nn.Linear(self.in_features, self.hidden_dim, bias=self.has_gate_bias)
        self.up_proj = nn.Linear(self.in_features, self.hidden_dim, bias=self.has_up_bias)
        self.down_proj = nn.Linear(self.hidden_dim, self.out_features, bias=self.has_down_bias)
    ...
    def _detect_mlp_structure(self, module: nn.Module):
        ...
        # Pattern 2: Sequential MLP (common in some architectures)
            ...
            if len(linear_layers) >= 2:
                ...
                self.up_proj_orig = linear_layers[1] if len(linear_layers) > 1 else None
        ...
        # Pattern 4: LLaMA/Mistral style MLP
            ...
            raise ValueError(f"Cannot detect MLP structure in module of type {type(module)}")
        
        # Ensure we have all required projections
        if not hasattr(self, 'gate_proj_orig'):
            raise ValueError("Could not identify gate projection in original module")
        if not hasattr(self, 'down_proj_orig'):
            raise ValueError("Could not identify down projection in original module")
    ...
    def _copy_weights(self, original_module: nn.Module):
        ...
        with torch.no_grad():
            ...
            if hasattr(self, 'gate_proj_orig'):
                ...
                if self.gate_proj.bias is not None and self.gate_proj_orig.bias is not None:
            ...
            if hasattr(self, 'up_proj_orig') and self.up_proj_orig is not None:
                ...
                if self.up_proj.bias is not None and self.up_proj_orig.bias is not None:
            ...
            if self.down_proj.bias is not None and self.down_proj_orig.bias is not None:
...
class SparseMLPWrapper(nn.Module):
    """
    Wrapper that makes any MLP sparse by applying top-k sparsity to its intermediate activations
    """
    
    def __init__(self, original_module: nn.Module, sparsity_ratio: float = 0.1, 
                 k: Optional[int] = None, sparse_gradients: bool = False):
        """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if hasattr(self.original_module, 'gate_proj') and hasattr(self.original_module, 'up_proj'):
...
def replace_mlp_modules(
    ...
    def replace_module(module, name=""):
        ...
        if target_names is not None:
            should_replace = any(target_name in name for target_name in target_names)
        elif module_type is not None:
            should_replace = isinstance(module, module_type)
        else:
            # Auto-detect MLP modules
            should_replace = (
                (hasattr(module, 'gate_proj') and hasattr(module, 'up_proj') and hasattr(module, 'down_proj')) or
                (hasattr(module, 'w1') and hasattr(module, 'w2') and hasattr(module, 'w3')) or
                (hasattr(module, 'fc1') and hasattr(module, 'fc2'))
            )
        
        if should_replace:
            print(f"Replacing module: {name} (type: {type(module).__name__})")
            return TritonSparseMLP(module, sparsity_ratio=sparsity_ratio, use_triton=use_triton)
        
        # Recursively process children
        for child_name, child in module.named_children():
            new_child = replace_module(child, f"{name}.{child_name}" if name else child_name)
...
def convert_llama_mlp(model: nn.Module, sparsity_ratio: float = 0.1) -> nn.Module:
    """
...
if __name__ == "__main__":
    ...
    # Count MLP modules
    mlp_count = sum(1 for _ in model.modules() if hasattr(_, 'w1') and hasattr(_, 'w2'))
    ...
    print(f"   New parameters: {sum(p.numel() for p in sparse_model.parameters()):,}")
    ...
    print(f"   ✓ Converted LLaMA-style MLPs with 20% sparsity")
