...
def set_up_sparsification(model, ablation_kind, importance_percentage,
                            sparse_implementation, device, k, n, m):
    if ablation_kind == "importance":
        assert 0 <= importance_percentage <= 100
        selected_amount = math.ceil((len(IMPORTANCE_RANKING) * importance_percentage) / 100)
